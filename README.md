...
This is a set of boilerplate scripts inspired by [GitHub](https://github.com/github/scripts-to-rule-them-all) 
that describes the normalized script pattern for BrieBug projects. These patterns are
intended to work for projects based on any framework or language. The examples included
here are for Node.js projects.
...
This can mean npm packages, Homebrew packages, Node versions, Git submodules, etc.
...
Linting (i.e. eslint, tslint, etc.) can also be considered a form of testing. These tend to run faster than tests, so put them towards the beginning of a [`script/test`][test] so it fails faster if there's a linting problem.
